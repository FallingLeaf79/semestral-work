...
public class Kalendar {
  ...
  public static final int LINE_LENGTH = 22;
  ...
  public static void printToCenter (String name, int lineLength) {
    repeatStuff(
      (lineLength / 2) - (name.length() / 2) - (name.length() % 2), ' '
    );
    System.out.print(name);
    repeatStuff(
      (lineLength / 2) - (name.length() / 2) - (lineLength % 2), ' '
    );
  }

  public static void printMonthLine(Calendar cal, char edge, int lineLength) {
    String name = (
      getMonthName(cal.get(Calendar.MONTH)) + " " +
      Integer.toString(cal.get(Calendar.YEAR))
    );
    System.out.print(edge);
    printToCenter(name, lineLength);
    System.out.println(edge);
  }

  public static void printCalendar(
    Calendar cal, char edge, char sep, int lineLength
  ) {
    printSeparator(lineLength, '.', sep);
    printMonthLine(cal, edge, lineLength);
    printSeparator(lineLength, edge, sep);
  ...
  public static void main(String[] args) {
    ...
    printCalendar(cal, EDGE, SEP, LINE_LENGTH);
  }
}
